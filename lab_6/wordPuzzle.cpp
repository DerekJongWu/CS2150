...
int main(int argc, char * argv[]) {
  string dict = argv[1]; 
  string puz = argv[2];
  hashTable tab = hashTable(dict);  
  int rows, cols;
  // attempt to read in the file
  bool result = readInGrid (puz, rows, cols);
  ...
  for ( int s1 = 0; s1 < rows; s1++ ){
    for ( int s2 = 0; s2 < cols; s2++ ) { 
      for (int l = 3; l < 23 ; l++) { 
	for (int i = 0; i < 8; i++) {
	  char *c = getWordInGrid(s1,s2,i,l,rows,cols);
	  string check = c; 
	  if (check.length() == l) { 
	  string out;
	  ...
	  if (tab.find(check) == true) { 
	    count++; 
	    fin = fin + out + " (" + to_string(s1) + ", " + to_string(s2) + ") : " + check + "\n";   
	    }
	    else { 
	      continue;  
	    }
  ...
  cout << endl; 
  cout << "Found all words in " << t << " seconds";
  cout << endl;
  cout << tot;  
  cout << endl; 
  return 0;
